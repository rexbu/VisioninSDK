...
-(void)initVideoFrame{
    ...
    /* 以下代码是加载第二个贴纸，一般用于用户给主播送礼物
    NSString* props2 = [[NSBundle mainBundle] pathForResource:@"lufei" ofType:@"zip"];
    [[VSProps shareInstance] startLocalProps2:props2 mirror:false];
    [[VSProps shareInstance] setPropsLoadBlock2:^{
        NSLog(@"Load2 Success!");
    }];
    */
    
    /* 以下代码是添加水印
    UIImage* logo = [UIImage imageNamed:@"watermark_logo.png"];
...
-(void)shaperValueChanged:(id)sender{
    ...
    [[VSFacer shareInstance] setShapping:SAHPER_CMD_CHEEK strength:strength];
}
-(void)setNoneSmooth{
...
//    }
//    else{
//        [[VSProps shareInstance] startProps:@"lufei" mirror:false];
//    }
}
